...
class Agent(object):
    ...
    def fit(self, env, nb_steps, action_repetition=1, callbacks=None, verbose=1,
            ...
            nb_max_episode_steps=None, version=None, custom_env=False):
        ...
        try:
            while self.step < nb_steps:
                ...
                if (custom_env):
                    metrics = self.backward(reward[0], terminal=done)  # tran's version
                else:
                    metrics = self.backward(reward, terminal=done)  # for testing with dqn_cartpole                
                ...
                if done:
                    ...
                    if(custom_env):
                        episode_logs = {
                            'episode_reward': episode_reward[0],  # Only return the first value
                        ...
                        episode_logs = {
                        'episode_reward': episode_reward,  # seems to return an array
                        'nb_episode_steps': episode_step,
                        'nb_steps': self.step,
                    }
